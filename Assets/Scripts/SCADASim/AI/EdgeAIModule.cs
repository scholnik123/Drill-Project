        ...
        private void Analyze(DrillingState state)
        {
            ...
            if (horizontalHole && highFatigue && lowFrequencyVibration && CanAlert(lastStickSlipAlertTime))
            {
                ...
                RaiseRecommendation(new AIRecommendation(
                    ...
                    "Снизить RPM на 10-15% и удерживать WOB без резких изменений.",
                    state.MeasuredDepth));
            }
            ...
            if (pressureTrendBarPerSecond > 0.42f && state.StuckPipeRisk01 > 0.52f && CanAlert(lastPackOffAlertTime))
            {
                ...
                RaiseRecommendation(new AIRecommendation(
                    ...
                    "Сальникообразование",
                    "SPP растет вместе с drag и риском прихвата.",
                    "Увеличить расход, снизить ROP, контролировать стабилизацию SPP.",
                    state.MeasuredDepth));
            }
            ...
            if (state.BoreholeInstabilityRisk01 > 0.68f && crew.Fatigue > 0.5f && CanAlert(lastInstabilityAlertTime))
            {
                ...
                RaiseRecommendation(new AIRecommendation(
                    ...
                    "Удерживать WOB/RPM, стабилизировать расход, исключить резкие команды.",
                    state.MeasuredDepth));
            }
            ...
            if (state.LostCirculationRisk01 > 0.6f && CanAlert(lastLossAlertTime))
            {
                ...
                RaiseRecommendation(new AIRecommendation(
                    AlertSeverity.Critical,
                    "Риск поглощения",
                    "ECD приближается к давлению гидроразрыва.",
                    "Снизить расход на 100-200 л/мин, контролировать объем в емкостях, подготовить LCM.",
                    state.MeasuredDepth));
            }
            ...
            if (state.BitWear01 > 0.75f && CanAlert(lastBitWearAlertTime))
            {
                ...
                RaiseRecommendation(new AIRecommendation(
                    ...
                    "Ограничить WOB/RPM, подготовить смену долота, не форсировать ROP.",
                    state.MeasuredDepth));
            }
        }
